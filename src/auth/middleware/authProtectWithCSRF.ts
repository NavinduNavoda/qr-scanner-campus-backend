import { CheckSession, CheckSessionAndCSRF } from '../../auth/functions/auth.check';
import { Request, Response, NextFunction } from 'express';

export const authProtectWithCSRF = (req: Request, res: Response, next: NextFunction) => {
    ...
    if(csrfTokenFromHeader && req.cookies["csrf"]){
        if(csrfTokenFromHeader !== req.cookies["csrf"]){
            ...
            if(req.cookies["session"] && CheckSessionAndCSRF(req.cookies["session"], csrfTokenFromHeader as string, "student")){
