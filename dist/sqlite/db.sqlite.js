...
export const addDefaultUserToUserTable = () => {
    ...
    const { count } = sqliteDB.prepare(userExistsQuery).get();
    const currentCountQuery = "SELECT COUNT(*) as count FROM current";
    const { count: currentCount } = sqliteDB.prepare(currentCountQuery).get();
    if (currentCount == 0) {
        const insertCurrentQuery = `
      INSERT INTO current (cash, cheque, credit, bill_to_bill, company_cheque)
      VALUES (0, 0, 0, 0, 0)
    `;
        sqliteDB.prepare(insertCurrentQuery).run();
    }
    if (count == 0) {
        registerNewUser({
            username: "devstudent",
